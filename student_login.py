...
class Login:
    ...
    def __init__(self,root):
        ...
        title = Label(frame1, text="STUDENT LOGIN ", font=("times in roman", 20, "bold"), bg="red", fg="black").place(
            x=50, y=30,width=800)

        mobile = Label(frame1, text="Mobile No.", font=("times in roman", 15, "bold"), bg="red", fg="black").place(x=100,
                                                                                                              y=100)
        self.txt_mobile = Entry(frame1, font=("times in roman", 15), bg="lightgray")
        self.txt_mobile.place(x=290, y=100, width=500)
        ...
        btn_login = Button(frame1, image=self.btn_imgss, bd=0, cursor="hand2", command=self.home_window).place(x=690,
                                                                                                               y=420)
    ...
    def login_data(self):
            if self.txt_mobile.get()=="" or self.txt_password.get()=="" or self.cmb_quests.get()=="" or self.txt_answers.get()=="":
                ...
                try:
                    ...
                    s = "SELECT * FROM details  WHERE Contact_no =%s AND Passwords=%s "
                    b1 = (self.txt_mobile.get(), self.txt_password.get())
                    cur.execute(s, b1)
                    row = cur.fetchone()
                    ...
                    if row ==None:
                        ...
                        messagebox.showinfo("Congratulations", "Login Successful,Click ok to move further process", parent=self.root)
                        self.root.destroy()
